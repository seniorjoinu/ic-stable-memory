...
mod btree_map_benchmark {
    ...
    use crate::collections::nrb_tree_map::NRBTreeMap;
    ...
    fn cmp_with_new() {
...

        {
            ...
            let mut nested_rb_tree_map = NRBTreeMap::new();

            measure!("Stable btree map insert", ITERATIONS, {
                for i in 2..ITERATIONS / 2 {
                    nested_rb_tree_map.insert(ITERATIONS / 2 + i, i);
                    nested_rb_tree_map.insert(ITERATIONS / 2 - i, i);
                }
            });
            ...
            measure!("Stable btree map search", ITERATIONS, {
                for i in 2..ITERATIONS / 2 {
                    nested_rb_tree_map.get_copy(&(ITERATIONS / 2 + i)).unwrap();
                    nested_rb_tree_map.get_copy(&(ITERATIONS / 2 - i)).unwrap();
                }
            });
            ...
            measure!("Stable btree map remove", ITERATIONS, {
                for i in 2..ITERATIONS / 2 {
                    nested_rb_tree_map.remove(&(ITERATIONS / 2 + i)).unwrap();
                    nested_rb_tree_map.remove(&(ITERATIONS / 2 - i)).unwrap();
                }
            });
        }
