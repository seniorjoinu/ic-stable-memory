...
impl StableMemoryAllocator {
    ...
    pub(crate) fn store(self) {
        ...
        for i in 0..SEG_CLASS_PTRS_COUNT as usize {
            if let Some(free_block) = self.seg_class_heads[i] {
                slice.as_bytes_write(offset, free_block.ptr);
            } else {
                slice.as_bytes_write(offset, EMPTY_PTR);
            }
            offset += PTR_SIZE;
        }
        ...
        slice.as_bytes_write(offset, self.free_size);
        offset += PTR_SIZE;

        slice.as_bytes_write(offset, self.allocated_size);
        offset += PTR_SIZE;

        for i in 0..CUSTOM_DATA_PTRS_COUNT {
            slice.as_bytes_write(offset, self.custom_data_ptrs[i]);
            offset += PTR_SIZE;
        }
    }
    ...
    pub(crate) unsafe fn reinit(ptr: u64) -> Self {
        ...
        for free_block in &mut seg_class_heads {
            let ptr = slice.as_bytes_read(offset);
        ...
        let free_size = slice.as_bytes_read(offset);
        offset += PTR_SIZE;

        let allocated_size = slice.as_bytes_read(offset);
        offset += PTR_SIZE;

        let mut custom_data_ptrs = [0u64; CUSTOM_DATA_PTRS_COUNT];
        for ptr in &mut custom_data_ptrs {
            *ptr = slice.as_bytes_read(offset);
            offset += PTR_SIZE;
        }
