...
use crate::utils::certification::{
    fork, fork_hash, labeled, labeled_hash, leaf, leaf_hash, pruned, AsHashTree, Hash, HashTree,
    EMPTY_HASH,
};
    ...
    [(); V::SIZE]: Sized,
{
    ...
    #[inline]
    fn write_keys_from_buf(&self, from_idx: usize, buf: &[u8]) {
    ...
    #[inline]
    fn write_values_from_buf(&self, from_idx: usize, buf: &[u8]) {
...
impl<K: StableAllocated + Ord, V: StableAllocated + AsHashTree> AsHashTree<usize>
    for LeafBTreeNode<K, V>
where
    [(); K::SIZE]: Sized,
    [(); V::SIZE]: Sized,
{
    fn root_hash(&self) -> Hash {
        ...
        let mut lh = labeled_hash(&k, &v.root_hash());

        for i in 1..len {
            k = self.read_key(i);
            v = V::from_fixed_size_bytes(&self.read_value(i));

            lh = fork_hash(&lh, &labeled_hash(&k, &v.root_hash()));
        }
